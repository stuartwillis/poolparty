...
name <- args[6]
outdir <- args[7]
minDP <- as.numeric(args[8])
popfile <- args[9]

outname <- sub('\\..*', '', name)
infile <- read.delim(name , header= F, sep = " ")
infile <- infile[,colSums(is.na(infile))<nrow(infile)]
pops<-read.delim(popfile , header= F, sep = " ")
pops<-as.character(pops$V1)
#pops<-as.character(droplevels(pops$V1))
  ...
  #Total mean coverage for populations 
  	#nrow(infile[ which(infile$V1=='TMA'), ])
  	tma <-   infile[ which(infile$V1=='TMA'), ]
  	tma<- tma[,1:3]
  	#table(droplevels(tma$V2))
  	tms <-  infile[ which(infile$V1=='TMS'), ]
  	#paste(tma[,2],tms[,2],sep=" ")
  	tma$V4 <- tms$V3
  	tma <- tma[,-1]
  	...
  	tma2<-as.data.frame(matrix(nrow=0,ncol=ncol(tma)))
  	for (p in 1:length(unique(tma$Population))) {
  	  tma2<-rbind(tma2,head(tma[tma$Population==unique(tma$Population)[p],],n=1))
  	}
  	tma<-tma2
  	tma <- tma[order(tma$Population),]
#    nrow(tma)
#  	length(unique(tma$Population))
  	pnumz <- as.numeric(nrow(tma))
...
#	  pnumz <- as.numeric(nrow(tms))
	...
	#plot
  	...
  	p <- q + labs(title= "Mean Depth of Coverage", x= "Library", y = "Mean Depth of Coverage (X)") + 
   	 theme(
     	 ...
     	 axis.text.x = element_text(angle = 90),axis.ticks.x=element_blank(),  panel.grid.major = element_blank(), panel.grid.minor = element_blank(),  panel.background = element_blank(),
  ...
  print("R ALERT: Mean Coverage Stats Complete")
  	...
  	####insert for loop to keep one line per pop
  	fma2<-as.data.frame(matrix(nrow=0,ncol=ncol(fma)))
  	for (p in 1:length(unique(fma$Population))) {
  	  fma2<-rbind(fma2,head(fma[fma$Population==unique(fma$Population)[p],],n=1))
  	}
  	fma<-fma2
...
#Proportion of genome covered at current filters
  ...
  ####insert for loop to keep one line per pop
  fpc2<-as.data.frame(matrix(nrow=0,ncol=ncol(fpc)))
  for (p in 1:length(unique(fpc$Population))) {
    fpc2<-rbind(fpc2,head(fpc[fpc$Population==unique(fpc$Population)[p],],n=1))
  }
  fpc<-fpc2
 ...
 #Anchored vs scaffold proportion of genome after filters  
	...
	#Proportion of genome covered at current filters
 		 ...
 		 ####insert for loop to keep one line per pop
 		 SCAFP2<-as.data.frame(matrix(nrow=0,ncol=ncol(SCAFP)))
 		 for (p in 1:length(unique(SCAFP$Population))) {
 		   SCAFP2<-rbind(SCAFP2,head(SCAFP[SCAFP$Population==unique(SCAFP$Population)[p],],n=1))
 		 }
 		 SCAFP<-SCAFP2
 		 ...
 		 ####insert for loop to keep one line per pop
 		 ANCP2<-as.data.frame(matrix(nrow=0,ncol=ncol(ANCP)))
 		 for (p in 1:length(unique(ANCP$V2))) {
 		   ANCP2<-rbind(ANCP2,head(ANCP[ANCP$V2==unique(ANCP$V2)[p],],n=1))
 		 }
 		 ANCP<-ANCP2
  	...
  	SCANC$Population <- c(rep(pops,each=2),"Mean","Mean")
 ...
 #Proportion of genome covered at different depths of coverage
  		...
  		dop$V3 <- factor(dop$V3, levels = unique(dop$V3))
  	...
  	colnames(df.melted) <- c("Coverage", "Library", "value")
		...
		#Plot
 		...
 		p <- ggplot(df.melted, aes(x = Coverage, y = value, colour = Library)) +  
 		  ...
 		  # Dotted line
 		  geom_vline(xintercept=minDP, color ="grey",  linetype = "longdash", lwd=.25)
 	 ...
 	 if(length(pops)>14){
 	   ...
 	   f <- f + theme(
 	     axis.text.x  = element_text(angle=90, vjust=0.5),
 	     panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(),
 	     panel.border = element_rect(colour = "black", fill=NA, size=.25),legend.position = "none") +
 	     scale_fill_manual( values= colfunc3(nrow(fpc)))
 	   
 	   #xaxis ticks
 	   #axis.ticks.x=element_blank()
 	   
 	   namez <- paste0(outname, "_prop_at_covs_noLEGEND.pdf")
 	   savePlot <- function(f) {
 	     pdf(namez, width=7, height=5)
 	     print(f)
 	     invisible(dev.off())
  ...
  ### CHROMOSOME PROPORTIONS ####
	...
	if (tnum < 100) {
 	 #..and now scaffolds
  		...
  		####insert for loop to keep one line per pop
  		scafz2<-as.data.frame(matrix(nrow=0,ncol=ncol(scafz)))
  		for (p in 1:length(unique(scafz$Population))) {
  		  scafz2<-rbind(scafz2,head(scafz[scafz$Population==unique(scafz$Population)[p],],n=1))
  		}
  		scafz<-scafz2
  		scafz$Population<-pops
  ...
  ###MEANS ONLY
  	...
  	#Get mean values for all populations first
  		...
  		rownames(mean.mchr)<-mean.mchr$Group.1
  		#  		mean.mchr <- mean.mchr[order(nchar(as.character(mean.mchr[[1]]))),]
  		mean.mchr <- mean.mchr[chrsize$chr,]
  		std.mchr <- aggregate(mchr[, 3], list(mchr$Chromosome), stdev)
  		#	  	std.mchr <- std.mchr[order(nchar(as.character(std.mchr[[1]]))),]
  		rownames(std.mchr)<-std.mchr$Group.1
  		std.mchr <- std.mchr[chrsize$chr,]
  ...
  #		sum(mean.mchr$size / sum(mean.mchr$size))
  #		sum(mean.mchr$prop)
  	...
  	if (length(popz) >1) {
    		pI=0
    			for (i in (popz)) {
      				...
      				scalehigh <- max(subz$Proportion)+ max(subz$Proportion) *.95 
      				scalelow <- 0
      				diff <- scalehigh - scalelow
			#TICK ADJUST#
	 		if (diff > 0 && diff < .05) {
